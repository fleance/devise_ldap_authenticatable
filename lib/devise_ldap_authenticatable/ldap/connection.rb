  ...
  module LDAP
    class Connection
      ...
      def initialize(params = {})
        ...
        DeviseLdapAuthenticatable::Logger.send("LDAP auth result: " + @ldap.get_operation_result.message)
      end
      ...
      def dn
        @dn ||= begin
        response = @ldap.search(:filter => Net::LDAP::Filter.eq(@attribute.to_s, @login.to_s))
        DeviseLdapAuthenticatable::Logger.send(pp response)
        return @login
